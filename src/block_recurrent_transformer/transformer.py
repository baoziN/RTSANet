...
def apply_rotary_pos_emb(t: torch.Tensor, freqs: torch.Tensor) -> torch.Tensor:
    ...
    # 如果 freqs 是标量（float），将其转换为张量
    if isinstance(freqs, float):
        freqs = torch.full((seq_len, 1), freqs, dtype=torch.float32)
    
    # 检查 freqs 是否为张量，且有至少两个维度
    if freqs.ndimension() == 1:
        freqs = freqs.unsqueeze(0)  # 将其转换为二维张量，例如 [1, seq_len]
    
    # 切片操作：确保 freqs 的维度大于或等于 2
    freqs = freqs[-seq_len:, :]
